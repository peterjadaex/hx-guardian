...
if [[ $? -eq 0 ]]; then
    printf '{"rule":"os_skip_apple_intelligence_enable","action":"EXECUTED","message":"Fix applied"}\n'
    exit 0
else
    printf '{"rule":"os_skip_apple_intelligence_enable","action":"FAILED","message":"defaults write failed"}\n'
    exit 1
fi
