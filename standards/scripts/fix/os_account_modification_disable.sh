...
if [[ $? -eq 0 ]]; then
    printf '{"rule":"os_account_modification_disable","action":"EXECUTED","message":"Fix applied"}\n'
    exit 0
else
    printf '{"rule":"os_account_modification_disable","action":"FAILED","message":"defaults write failed"}\n'
    exit 1
fi
