...
if [[ "$result_value" == "$expected_value" ]]; then
    printf '{"rule":"os_sshd_permit_root_login_configure","status":"PASS","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 0
else
    printf '{"rule":"os_sshd_permit_root_login_configure","status":"FAIL","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 1
fi
