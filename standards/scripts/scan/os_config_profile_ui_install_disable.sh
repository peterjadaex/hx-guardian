...
if [[ "$result_value" == "$expected_value" ]]; then
    printf '{"rule":"os_config_profile_ui_install_disable","status":"PASS","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 0
else
    printf '{"rule":"os_config_profile_ui_install_disable","status":"FAIL","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 1
fi
