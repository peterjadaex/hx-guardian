...
if [[ "$result_value" == "$expected_value" ]]; then
    printf '{"rule":"os_skip_unlock_with_watch_enable","status":"PASS","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 0
else
    printf '{"rule":"os_skip_unlock_with_watch_enable","status":"FAIL","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 1
fi
