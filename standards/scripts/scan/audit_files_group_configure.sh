...
if [[ $EUID -ne 0 ]]; then
    printf '{"rule":"audit_files_group_configure","status":"ERROR","message":"Must be run as root"}\n'
    exit 3
fi
...
if [[ "$result_value" == "$expected_value" ]]; then
    printf '{"rule":"audit_files_group_configure","status":"PASS","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 0
else
    printf '{"rule":"audit_files_group_configure","status":"FAIL","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 1
fi
