...
if [[ "$result_value" == "$expected_value" ]]; then
    printf '{"rule":"pwpolicy_account_inactivity_enforce","status":"PASS","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 0
else
    printf '{"rule":"pwpolicy_account_inactivity_enforce","status":"FAIL","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 1
fi
