...
if [[ $EUID -ne 0 ]]; then
    printf '{"rule":"system_settings_guest_account_disable","status":"ERROR","message":"Must be run as root"}\n'
    exit 3
fi
...
if [[ "$result_value" == "$expected_value" ]]; then
    printf '{"rule":"system_settings_guest_account_disable","status":"PASS","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 0
else
    printf '{"rule":"system_settings_guest_account_disable","status":"FAIL","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 1
fi
