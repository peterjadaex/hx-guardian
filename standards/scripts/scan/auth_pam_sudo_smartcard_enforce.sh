...
if [[ "$result_value" == "$expected_value" ]]; then
    printf '{"rule":"auth_pam_sudo_smartcard_enforce","status":"PASS","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 0
else
    printf '{"rule":"auth_pam_sudo_smartcard_enforce","status":"FAIL","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 1
fi
