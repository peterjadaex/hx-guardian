...
if [[ "$result_value" == "$expected_value" ]]; then
    printf '{"rule":"os_system_wide_applications_configure","status":"PASS","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 0
else
    printf '{"rule":"os_system_wide_applications_configure","status":"FAIL","result":"%s","expected":"%s"}\n' "$result_value" "$expected_value"
    exit 1
fi
